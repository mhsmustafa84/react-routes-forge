...
describe('defineRoutes', () => {
  ...
  it('builds dynamic paths correctly', () => {
    ...
    expect((PATHS.USERS.DETAILS as any).build({ id: 'abc' })).toBe('/users/abc');
  });
...
describe('isActivePath', () => {
  ...
  it('matches prefixes when exact is false', () => {
    expect(isActivePath('/users/42/posts', '/users/:id', { exact: false })).toBe(true);
  });
...
describe('extractParamsFromPath', () => {
  ...
  it('extracts multiple params', () => {
    expect(
      extractParamsFromPath('/users/:userId/posts/:postId', '/users/1/posts/99')
